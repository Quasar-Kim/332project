...
DIR_OUTPUT="/home/red/edge_case_output/skew"
DIR_INPUT="/home/red/edge_case_input/skew"
DIR_SUMMARIES="/home/red/edge_case_summaries/skew"
...
##### GENERATE DATA #####
data() {
    local tag=$1 # either empty or "-a"

    local inp_parent="$DIR_INPUT/$N-machines/$DIRS-dirs-$FILES-files-$RECORDS-$REC_TYPE-recs"
    local out_dir="$DIR_OUTPUT/$N-machines/$DIRS-dirs-$FILES-files-$RECORDS-$REC_TYPE-recs"
    ...
    for worker in ${WORKER_IPS[@]:0:$N}; do
        ...
        for ((i=0; i<DIRS; i++)); do
            ...
            for ((j=0; j<FILES; j++)); do
                ssh $USER@$worker -p $WORKER_SSH_PORT \
                    "$GENSORT_PATH -s $tag $RECORDS $inp_parent/dir$i/file$j"
            done
        done
    done
    ...
    generate_commands "$dir_string" "$out_dir" "$DIR_SUMMARIES/$TYPE/$N-machines/$DIRS-dirs-$FILES-files-$RECORDS-$REC_TYPE-recs" $rec_cnt
}

usage() {
    echo "The command creates the needed directories and files, and outputs the commands to perform the test"
    echo ""
    echo "Skewed ascii records: ./skew.sh ascii <# workers> <# directories> <# files per directory> <# records per file>"
    echo "Skewed binary records: ./skew.sh binary <# workers> <# directories> <# files per directory> <# records per file>"
}
