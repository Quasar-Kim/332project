...
import redsort.jobs.worker.filestorage.InMemoryFileStorage
import cats.effect._
import fs2._
import java.io.FileNotFoundException

class InMemoryFileStorageSpec extends AsyncFlatSpec with AsyncIOSpec with Matchers {
  ...
  it should "list files with a given prefix" in {
    withStorage { storage =>
      for {
        ...
        files <- storage.list("/tmp/folder").compile.toList
      } yield {
